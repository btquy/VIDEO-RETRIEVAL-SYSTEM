...
def to_image_tokens_v2(image_paths):
    # Load images from URLs and convert to tensors (modify according to your tokenization process)
    img_tensors = []
    for path in image_paths:
        img = load_image_from_url(path)
        transform = build_transform(384)
        img_tensor = transform(img)  # Apply your image transformations here
        img_tensors.append(img_tensor)
    return img_tensors

def calc_img_embedding_v2(image_paths):
    output_dict = dict()
    img_tokens = to_image_tokens_v2(image_paths)
    img_tokens_stacked = torch.stack(img_tokens, dim=0)
    img_tokens_stacked = img_tokens_stacked.to(device)
    with torch.no_grad():
        img_embedding = model(image=img_tokens_stacked, only_infer=True)
...
def image_embedding_v2(image_path, top_k=20, object_input=None):
    index = index_pinecone_embedding()
    img_embedding = calc_img_embedding_v2(image_path)
    img_embedding_list = img_embedding[image_path[0]].cpu().detach().numpy().tolist()
    ...
    if object_input is not None:    
        filter={
            ...
            "object_count": { "$in": object_input["object_count"] },
            "object_colors": { "$in": object_input["object_color"] },
            "object_locations": { "$in": object_input["object_location"] }  # Corrected typo here
        }
        query_results = index.query(
            vector=img_embedding_list,
            top_k=top_k, 
