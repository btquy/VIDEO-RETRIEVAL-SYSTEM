...
def query_keyframe_in_video_segment(text_query, video_name, frame_start, frame_end):
    ...
    file_path113 = 'output113.txt'

    with open(file_path113, 'w') as file:
        print(query_results, file=file)
...
def transcript(text):
    ...
    if matches is not None and len(matches) > 0:
        for item in matches:
            ...
            print(text, video_name, frame_start, frame_end)
    ...
    with open(file_path1, 'w') as file:
        print(keyframe_result, file=file)

    with open(file_path2, 'w') as file:
        print(finalDict, file=file)

    with open(file_path3, 'w') as file:
        print(base_result, file=file)
